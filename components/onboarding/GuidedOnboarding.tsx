...
import {
  ...
  User as UserIcon, Sparkles, Plus, Trash2,
  Building2, Clock, DollarSign, Mail, X, Bell, MessageSquare,
} from "lucide-react";
import {
  BUSINESS_TEMPLATES, getTemplate, DEFAULT_REMINDER_MESSAGE,
  type BusinessTemplate, type TemplateAvailability,
} from "@/lib/business-templates";

// ─── Types ──────────────────────────────────────────────────────────────────
type Service = { key: string; title: string; duration: number; price: number; color: string; capacity: number; enabled: boolean };
...
function weekDefault(fromDay: number, toDay: number, start: string, end: string): DayAvail[] {
  const wraps = fromDay > toDay;
  return Array.from({ length: 7 }, (_, d) => ({
    dayOfWeek: d,
    isActive: wraps ? (d >= fromDay || d <= toDay) : (d >= fromDay && d <= toDay),
    startTime: start,
    endTime: end,
  }));
}
function availFromTemplate(a: TemplateAvailability): DayAvail[] {
  return weekDefault(a.fromDay, a.toDay, a.start, a.end);
}
...
export default function GuidedOnboarding({ userName }: { userName: string }) {
  ...
  const template = useMemo<BusinessTemplate | null>(() => (templateId ? getTemplate(templateId) ?? null : null), [templateId]);
  ...
  const isOrg = !!template?.isOrg;
  const hasPrices = !!template?.hasPrices;
  const hasDepartments = !!template?.hasDepartments;

  function chooseTemplate(id: string) {
    const t = getTemplate(id);
    if (!t) return;
    setTemplateId(id);
    setServices(t.services.map((s) => ({ ...s, key: uid(), enabled: true, capacity: s.capacity ?? 1 })));
    setDepartments((t.departments ?? []).map((d) => ({ ...d, key: uid(), enabled: true })));
    setAvailability(availFromTemplate(t.availability));
    setReminderHours(t.defaultReminderHours);
  ...
  // Dynamic step list for the chosen template
  const steps = useMemo(() => {
    if (!template) return ["usecase"];
    const s = ["usecase"];
    if (isOrg) s.push("org");
    s.push("profile");
    if (hasDepartments) s.push("departments");
    s.push("services", "availability", "reminders");
    if (isOrg) s.push("invite");
    s.push("review");
    return s;
  }, [template, isOrg, hasDepartments]);
    ...
    try {
      const payload = {
        ...
        services: services.filter((s) => s.enabled && s.title.trim()).map((s) => ({
          ...
          slotInterval: template?.slotInterval ?? 0,
        })),
        ...
        workflow: reminderEnabled ? {
          name: `${reminderHours >= 24 ? `${reminderHours / 24}-day` : `${reminderHours}-hour`} reminder`,
          trigger: "BEFORE",
          offsetMinutes: reminderHours * 60,
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#f4f6fb" }}>
      <header className="px-4 sm:px-8 py-4 flex items-center justify-between border-b border-gray-100 bg-white">
        ...
        {template && <span className="text-xs text-gray-400 font-medium">Step {stepIdx + 1} of {totalSteps}</span>}
      </header>
      ...
      <main className="flex-1 flex items-start justify-center px-4 py-8 sm:py-12 overflow-y-auto">
        <div className="w-full max-w-2xl">
          {currentStep === "usecase" && <TemplatePicker onChoose={chooseTemplate} />}

          {currentStep === "org" && (
            <OrgStep template={template!} orgName={orgName} setOrgName={setOrgName} orgSlug={orgSlug} setOrgSlug={setOrgSlug} />
          )}

          {currentStep === "profile" && (
            <ProfileStep isOrg={isOrg} username={username} setUsername={setUsername}
              displayName={displayName} setDisplayName={setDisplayName}
              bio={bio} setBio={setBio} timezone={timezone} setTimezone={setTimezone} />
          )}
          ...
          {currentStep === "services" && (
            <ServicesStep hasPrices={hasPrices} services={services} setServices={setServices} />
          )}
          ...
          {currentStep === "reminders" && (
            <RemindersStep
              ...
              hours={reminderHours} setHours={setReminderHours}
              channel={reminderChannel} setChannel={setReminderChannel}
            />
          )}
          ...
          {currentStep === "review" && (
            <ReviewStep template={template!} isOrg={isOrg} hasPrices={hasPrices} hasDepartments={hasDepartments}
              ...
              reminderEnabled={reminderEnabled} reminderHours={reminderHours} reminderChannel={reminderChannel} />
          )}
          ...
          {currentStep !== "usecase" && (
            <div className="flex items-center gap-3 mt-8">
              <button onClick={back} className="flex items-center gap-1.5 px-5 py-3 rounded-xl text-sm font-semibold border-2 border-gray-200 text-gray-600 hover:bg-white transition-colors">
              ...
              {currentStep === "review" ? (
                ...
                <>
                  {currentStep === "invite" && (
                    <button onClick={next} className="px-5 py-3 rounded-xl text-sm font-semibold text-gray-500 hover:text-gray-700 transition-colors">Skip for now</button>
                  )}
...
function TemplatePicker({ onChoose }: { onChoose: (id: string) => void }) {
  ...
  return (
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">Pick the type of business that fits you — we'll set up your services, hours and reminders. You can change anything later.</p>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {items.map((t) => (
              <button key={t.id} onClick={() => onChoose(t.id)}
                className="bg-white rounded-2xl border-2 border-gray-100 p-4 text-left hover:shadow-md hover:border-indigo-200 transition-all group flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <h3 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{t.name}</h3>
      ...
      <div className="mt-2">
        <button onClick={() => onChoose(custom.id)}
          className="w-full bg-white rounded-2xl border-2 border-dashed border-gray-200 p-4 text-left hover:border-indigo-300 hover:bg-indigo-50/30 transition-all group flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl flex-shrink-0 bg-gray-50">{custom.icon}</div>
          <div>
            <h3 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{custom.name}</h3>
            <p className="text-xs text-gray-500 mt-0.5">{custom.description}</p>
          </div>
        </button>
      </div>
    </div>
...
// ─── Step: Organization ───────────────────────────────────────────────────────
function OrgStep({ template, orgName, setOrgName, orgSlug, setOrgSlug }: any) {
  return (
    <div>
      <StepHeader icon={Building2} title={`Tell us about your ${template.name.toLowerCase()}`}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        <Field label="Business Name">
          <input value={orgName}
            ...
            placeholder="e.g. City Center Studio"
            className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400" autoFocus />
        </Field>
        <Field label="Public Page URL">
          <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-400">
            ...
            <input value={orgSlug} onChange={(e) => setOrgSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))}
              placeholder="city-center" className="flex-1 px-3 py-3 text-sm focus:outline-none" />
          </div>
        </Field>
      </div>
    </div>
...
function ProfileStep({ isOrg, username, setUsername, displayName, setDisplayName, bio, setBio, timezone, setTimezone }: any) {
  return (
    <div>
      <StepHeader icon={UserIcon} title={isOrg ? "Your personal booking link" : "Your booking link"}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        <Field label="Booking Link">
          <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-400">
            ...
            <input value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, ""))}
              placeholder="yourname" maxLength={30} className="flex-1 px-3 py-3 text-sm focus:outline-none" autoFocus />
          </div>
        </Field>
        <Field label="Display Name">
          <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Dr. Jane Smith"
        ...
        <Field label="Short Bio (optional)">
          <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={2} placeholder="A line or two about you or your services..."
  ...
  return (
    <div>
      <StepHeader icon={Building2} title="Set up your departments"
        subtitle="Group your staff by specialty. We've suggested a few — keep, edit, or remove them." />
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-3">
        {departments.map((d) => (
          <div key={d.key} className="flex items-center gap-3">
            ...
            <input value={d.name} onChange={(e) => update(d.key, { name: e.target.value })} placeholder="Department name"
            ...
            <button onClick={() => remove(d.key)} className="p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 size={16} /></button>
          </div>
        ))}
        <button onClick={add} className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 mt-2"><Plus size={16} /> Add department</button>
      </div>
    </div>
  );
}
...
// ─── Step: Services ───────────────────────────────────────────────────────────
function ServicesStep({ hasPrices, services, setServices }: { hasPrices: boolean; services: Service[]; setServices: (s: Service[]) => void }) {
  const add = () => setServices([...services, { key: uid(), title: "", duration: 30, price: 0, capacity: 1, color: COLORS[services.length % COLORS.length], enabled: true }]);
  ...
  return (
    <div>
      <StepHeader icon={CalendarCheck} title="What can people book?"
        subtitle="We've pre-filled common ones — customize freely. Toggle off any you don't offer." />
        ...
        {services.map((s) => (
          <div key={s.key} className={`rounded-xl border p-3 transition-colors ${s.enabled ? "border-gray-200" : "border-gray-100 bg-gray-50 opacity-60"}`}>
            <div className="flex items-center gap-3">
              ...
              <input value={s.title} onChange={(e) => update(s.key, { title: e.target.value })} placeholder="Service name"
              ...
              <button onClick={() => remove(s.key)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors flex-shrink-0"><Trash2 size={15} /></button>
            </div>
            <div className="flex items-center gap-2 mt-2 pl-8 flex-wrap">
              <label className="flex items-center gap-1.5 text-xs text-gray-500">
                <Clock size={13} />
                <input type="number" value={s.duration} min={5} step={5} onChange={(e) => update(s.key, { duration: parseInt(e.target.value) || 0 })}
                  className="w-16 px-2 py-1 rounded-md border border-gray-200 text-sm focus:outline-none focus:border-indigo-400" /> min
              </label>
              {hasPrices && (
                <label className="flex items-center gap-1 text-xs text-gray-500 ml-1">
                  <DollarSign size={13} />
                  <input type="number" value={s.price} min={0} onChange={(e) => update(s.key, { price: parseFloat(e.target.value) || 0 })}
                    className="w-20 px-2 py-1 rounded-md border border-gray-200 text-sm focus:outline-none focus:border-indigo-400" />
                </label>
              )}
              {s.capacity > 1 && (
                <span className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600">Group · {s.capacity} seats</span>
              )}
            </div>
          </div>
        ))}
        <button onClick={add} className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 mt-1"><Plus size={16} /> Add another</button>
      </div>
    </div>
  );
}
...
function AvailabilityStep({ availability, setAvailability }: { availability: DayAvail[]; setAvailability: (a: DayAvail[]) => void }) {
  const update = (day: number, patch: Partial<DayAvail>) => setAvailability(availability.map((d) => d.dayOfWeek === day ? { ...d, ...patch } : d));
  return (
    <div>
      <StepHeader icon={Clock} title="When are you available?" subtitle="Set your working hours. Clients can only book during these times." />
      ...
      }} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium mt-3">Apply Monday's hours to all active days</button>
    </div>
  );
}

// ─── Step: Reminders (default workflow) ────────────────────────────────────────
function RemindersStep({ enabled, setEnabled, hours, setHours, channel, setChannel }: {
  enabled: boolean; setEnabled: (b: boolean) => void;
  hours: number; setHours: (n: number) => void;
  channel: "EMAIL" | "SMS"; setChannel: (c: "EMAIL" | "SMS") => void;
}) {
  return (
    <div>
      <StepHeader icon={Bell} title="Automatic reminders"
        subtitle="Reduce no-shows by reminding clients before their booking. You can add more workflows later." />
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        <label className="flex items-center justify-between cursor-pointer">
          <span className="text-sm font-medium text-gray-800">Send a reminder before each booking</span>
          <button onClick={() => setEnabled(!enabled)}
            className={`w-10 h-6 rounded-full flex items-center transition-colors flex-shrink-0 ${enabled ? "justify-end" : "justify-start bg-gray-200"}`}
            style={enabled ? { backgroundColor: "#4F46E5" } : {}}>
            <span className="w-5 h-5 rounded-full bg-white shadow-sm mx-0.5" />
          </button>
        </label>
          ...
          <>
            <Field label="When">
              <select value={hours} onChange={(e) => setHours(parseInt(e.target.value))}
                className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400">
                {REMINDER_OPTIONS.map((o) => <option key={o.hours} value={o.hours}>{o.label}</option>)}
              </select>
            </Field>
            <Field label="How">
              <div className="grid grid-cols-2 gap-3">
                {(["EMAIL", "SMS"] as const).map((c) => (
                  <button key={c} onClick={() => setChannel(c)}
                    className={`flex items-center justify-center gap-2 py-3 rounded-xl border-2 text-sm font-medium transition-colors ${channel === c ? "border-indigo-400 text-indigo-600 bg-indigo-50" : "border-gray-200 text-gray-600"}`}>
                    {c === "EMAIL" ? <Mail size={15} /> : <MessageSquare size={15} />} {c === "EMAIL" ? "Email" : "SMS"}
                  </button>
                ))}
              </div>
            </Field>
            {channel === "SMS" && <p className="text-xs text-gray-400">SMS requires the Pro plan and a connected SMS provider.</p>}
          </>
        )}
      </div>
    </div>
        ...
        {invites.map((inv) => (
          <div key={inv.key} className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input type="email" value={inv.email} onChange={(e) => update(inv.key, e.target.value)} placeholder="colleague@example.com"
            ...
            {invites.length > 1 && (
              <button onClick={() => remove(inv.key)} className="p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"><X size={16} /></button>
            )}
          </div>
        ))}
        <button onClick={add} className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 mt-1"><Plus size={16} /> Add another</button>
...
// ─── Step: Review ─────────────────────────────────────────────────────────────
function ReviewStep({ template, isOrg, hasPrices, hasDepartments, username, orgName, orgSlug, services, departments, availability, timezone, invites, reminderEnabled, reminderHours, reminderChannel }: any) {
  const activeDays = availability.filter((d: DayAvail) => d.isActive);
  const reminderLabel = REMINDER_OPTIONS.find((o) => o.hours === reminderHours)?.label ?? `${reminderHours}h before`;
  return (
    <div>
      <StepHeader icon={Sparkles} title="You're all set! 🎉" subtitle="Here's a summary of your setup. Click finish to launch your booking system." />
      <div className="space-y-3">
        {isOrg && (
          <ReviewCard icon={Building2} title={template.name}>
        ...
        {hasDepartments && departments.length > 0 && (
          <ReviewCard icon={Building2} title={`${departments.length} departments`}>
            <div className="flex flex-wrap gap-1.5">
              {departments.map((d: Dept) => <span key={d.key} className="px-2 py-0.5 rounded-full text-xs font-medium text-white" style={{ backgroundColor: d.color }}>{d.name}</span>)}
            </div>
          </ReviewCard>
        )}
          ...
          <div className="space-y-1">
            {services.map((s: Service) => (
              <div key={s.key} className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span className="flex-1">{s.title}{s.capacity > 1 ? ` · group of ${s.capacity}` : ""}</span>
        ...
        <ReviewCard icon={Clock} title="Working hours">
          <div className="flex flex-wrap gap-1.5">
            {activeDays.map((d: DayAvail) => <span key={d.dayOfWeek} className="px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-600">{DAY_SHORT[d.dayOfWeek]} {d.startTime}–{d.endTime}</span>)}
          </div>
        </ReviewCard>
        <ReviewCard icon={Bell} title="Reminders">
          {reminderEnabled
            ? <p className="text-sm text-gray-600">{reminderChannel === "SMS" ? "SMS" : "Email"} reminder sent <span className="font-medium">{reminderLabel}</span></p>
            : <p className="text-sm text-gray-400">No automatic reminder (you can add one later)</p>}
        </ReviewCard>
        {isOrg && invites.length > 0 && (
          <ReviewCard icon={Mail} title={`${invites.length} team ${invites.length === 1 ? "invite" : "invites"}`}>
            <div className="flex flex-wrap gap-1.5">
              {invites.map((i: Invite) => <span key={i.key} className="px-2 py-0.5 rounded-full text-xs bg-blue-50 text-blue-600">{i.email}</span>)}
            </div>
          </ReviewCard>
        )}
      </div>
...
function ReviewCard({ icon: Icon, title, children }: { icon: any; title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 flex gap-3">
      <div className="w-9 h-9 rounded-xl bg-gray-50 flex items-center justify-center flex-shrink-0"><Icon size={18} className="text-gray-400" /></div>
